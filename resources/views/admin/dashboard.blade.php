@extends('layouts.admin')

@section('content')
<div class="space-y-8">
    <!-- Header -->
    <x-admin.page-header 
        title="Admin Dashboard"
        subtitle="Manage your MiniMeet tenant users and system overview."
    />

    <!-- Stats Grid -->
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <!-- Total Users -->
        <x-admin.stats-card 
            title="Total Tenant Users"
            :value="$stats['total_users'] ?? 0"
            color="green"
            :trend="$stats['new_users_today'] ?? 0"
            trend-label="new today"
            :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M15.75 17.25 12 21m0 0-3.75-3.75M12 21V3\' />'"
        />

        <!-- Active Users -->
        <x-admin.stats-card 
            title="Active Users"
            :value="$stats['active_users'] ?? 0"
            color="gray"
            :subtitle="round((($stats['active_users'] ?? 0) / max($stats['total_users'] ?? 1, 1)) * 100, 1) . '% of total'"
        />

        <!-- Total Tenants -->
        <x-admin.stats-card 
            title="Total Tenants"
            :value="$stats['total_tenants'] ?? 0"
            color="blue"
            :trend="$stats['active_tenants'] ?? 0"
            trend-label="active"
            :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M3.75 21h16.5M4.5 3h15l-.75 18H5.25L4.5 3Z\' />'"
        />

        <!-- Recent Activity -->
        <x-admin.stats-card 
            title="Activities (24h)"
            :value="$stats['recent_activities'] ?? 0"
            color="indigo"
            trend-label="last 24 hours"
            :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z\' />'"
        />
    </div>

    <!-- Recent Users & Quick Actions -->
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
        <!-- Recent Users -->
        <x-admin.card>
            <x-slot name="header">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg leading-6 font-medium text-gray-900">Recent Users</h3>
                    <a href="{{ route('admin.tenant-users.index') }}" 
                       class="text-sm font-medium text-blue-600 hover:text-blue-500">
                        View all →
                    </a>
                </div>
            </x-slot>
            
            @if(!empty($recentUsers))
                <div class="flow-root">
                    <ul role="list" class="-my-3">
                        @foreach($recentUsers as $user)
                        <li class="py-3">
                            <div class="flex items-center space-x-3">
                                <div class="h-8 w-8 rounded-full bg-gray-200 flex items-center justify-center">
                                    <span class="text-sm font-medium text-gray-700">
                                        {{ strtoupper(substr($user['name'], 0, 1)) }}
                                    </span>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-medium text-gray-900 truncate">{{ $user['name'] }}</p>
                                    <p class="text-sm text-gray-500 truncate">{{ $user['email'] }}</p>
                                </div>
                                <div class="flex flex-col items-end">
                                    <x-admin.status-badge :status="$user['is_active'] ? 'active' : 'inactive'" />
                                    <span class="text-xs text-gray-400 mt-1">{{ $user['role'] }}</span>
                                </div>
                            </div>
                        </li>
                        @endforeach
                    </ul>
                </div>
            @else
                <p class="text-sm text-gray-500">No recent users found.</p>
            @endif
        </x-admin.card>

        <!-- Quick Actions -->
        <x-admin.card header="Quick Actions">
            <div class="space-y-4">
                <x-admin.button 
                    variant="primary"
                    :href="route('admin.tenant-users.create')"
                    class="w-full justify-center"
                    :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M18 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0ZM3 19.235v-.11a6.375 6.375 0 0 1 12.75 0v.109A12.318 12.318 0 0 1 9.374 21c-2.331 0-4.512-.645-6.374-1.766Z\' />'"
                >
                    Create New User
                </x-admin.button>
                
                <x-admin.button 
                    variant="secondary"
                    :href="route('admin.tenant-users.index') . '?status=inactive'"
                    class="w-full justify-center"
                    :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M18.364 18.364A9 9 0 0 0 5.636 5.636m12.728 12.728A9 9 0 0 1 5.636 5.636m12.728 12.728L5.636 5.636\' />'"
                >
                    View Inactive Users
                </x-admin.button>
                
                @if(auth()->user() && auth()->user()->isSuperAdmin())
                <x-admin.button 
                    variant="secondary"
                    :href="route('admin.system-stats')"
                    class="w-full justify-center"
                    :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' d=\'M7.5 14.25v2.25m3-4.5v4.5m3-6.75v6.75m3-9v9M6 20.25h12A2.25 2.25 0 0 0 20.25 18V6A2.25 2.25 0 0 0 18 3.75H6A2.25 2.25 0 0 1 3.75 6v12A2.25 2.25 0 0 0 6 20.25Z\' />'"
                >
                    System Statistics
                </x-admin.button>
                @endif
            </div>
        </x-admin.card>
    </div>

    @if(!empty($systemHealth))
    <!-- System Health -->
    <x-admin.card header="System Health">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            @foreach(['database', 'cache', 'queue'] as $service)
                @php
                    $health = $systemHealth[$service] ?? null;
                    $isHealthy = (is_array($health) && ($health['status'] ?? '') === 'healthy') || $health === 'healthy';
                @endphp
                <div class="text-center p-4 bg-gray-50 rounded-lg">
                    <div class="text-2xl font-bold {{ $isHealthy ? 'text-green-600' : 'text-red-600' }}">
                        {{ $isHealthy ? '✓' : '✗' }}
                    </div>
                    <div class="text-sm text-gray-500 capitalize">{{ $service }}</div>
                </div>
            @endforeach
        </div>
    </x-admin.card>
    @endif
</div>
@endsection